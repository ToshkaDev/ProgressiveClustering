...
def clusterDirectonsRegardlessOfOperons(result):
	...
	for protein, neighbors in result.items():
		if neighbors[-1]["version"] != None and neighbors[-1]["version"] in PROTEIN_IDS:
			mainProteinStrand = neighbors[-1]["strand"]
			for neighbor in neighbors[:-1]:
				if neighbor["version"] != None and neighbor["strand"] == mainProteinStrand and "clusterId" in neighbor: 
...
def saveToFile(clusterNumberToMainProteinIds):
	with open(OUTPUT_FILE, "w")	as outputFile:
		for clstNumber, proteinIds in clusterNumberToMainProteinIds.items():
			proteinIdsSorted = sorted([(int(prot.split("_")[0]), prot) for prot in proteinIds], key=lambda protein: protein[0])
			for proteinId in proteinIdsSorted:
				outputFile.write(str(clstNumber) + "\t" + proteinId[1] + "\n")
