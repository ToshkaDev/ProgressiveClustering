...
USAGE = "\nThe script makes sequence names and/or tree leaves names newick friendly.\n" + \
	...
	[-l || --saveToFile]    - save or not to file. Default is "true"
	'''

def initialize(argv):
	global INPUT_FILE, INPUT_FILE_TREE, OUTPUT_FILE, OUTPUT_FILE_TREE, FETCH_FROM_IDS, FETCH_FROM_TREE
	global NUMBER_OF_PROCESSES, FETCH_FROM_MIST, FETCH_FROM_NCBI, REMOVE_DASHES, SAVE_TO_FILE
	try:
		opts, args = getopt.getopt(argv[1:],"hi:s:o:n:f:t:c:m:b:d:l:",["help", "ifile=", "sfile=", "ofile=", "nfile=", "fetchFromIds=", \
			"fetchFromTree=", "proc_num=", "fetchFromMist=", "fetchFromNCBI=", "removeDashes=", "saveToFile="])
		if len(opts) == 0:
		...
		if not handle and not proteinIdToSeq:
			...
			with open(INPUT_FILE, "r") as inputFile:
				for sequence in SeqIO.parse(inputFile, "fasta"):
					...
					#outputFile.write(">" + sequence.description + "\n")
		...
		if handle:
			...
			for eachRecord in SeqIO.parse(handle, "fasta"):
				if eachRecord.id not in savedIds:
					...
					#outputFile.write(">" + eachRecord.description + "\n")
					outputFile.write(protSeq + "\n")
		...
		if proteinIdToSeq:
			...
			for proteinName, seq in proteinIdToSeq.items():
				if proteinIdToTrueId[proteinName] not in savedIds:
					...
					#outputFile.write(">" + proteinName + "\n")
					outputFile.write(protSeq + "\n")
					savedIds.add(proteinIdToTrueId[proteinName])


# If the script is used inside another script (SAVE_TO_FILE == False)
def getChangedNamesAndSeqs(handle, proteinIdToSeq, proteinIdToTrueId):
	protNameToSeq = {}
	savedIds = set()
	if handle:
		#after retrieving seqeunces by Id from NCBI
		...
		for eachRecord in SeqIO.parse(handle, "fasta"):
			if eachRecord.id not in savedIds:
				...
				if REMOVE_DASHES:
					protSeq = protSeq.replace("-", "")
				protNameToSeq[getChangedName(eachRecord.description)] = protSeq
				savedIds.add(eachRecord.id)
	if proteinIdToSeq:
		#after retrieving seqeunces by Id from Mist
		for proteinName, seq in proteinIdToSeq.items():
			if proteinIdToTrueId[proteinName] not in savedIds:
				...
				protNameToSeq[getChangedName(eachRecord.description)] = protSeq
				savedIds.add(proteinIdToTrueId[proteinName])	
	return protNameToSeq


####===================================================#####
### Fetching sequences from NCBI using input Id list ###

def prepareIdListFromInput(inputData=False):
	proteinIds = set()
	if SAVE_TO_FILE:
		with open(INPUT_FILE, "r") as inputFile:
	...
	if inputData:
		for proteinId in inputData:
			proteinIds.add(proteinId.strip())
	proteinIdsMultiProc = manager.list(proteinIds.copy())
	return (proteinIds, proteinIdsMultiProc)


def fetchNamesAndSave(inputData=False):
	global SAVE_TO_FILE
	if inputData:
		SAVE_TO_FILE = False
		protNameToSeq = None
	if not inputData and not INPUT_FILE:
		return
	handle = None
	proteinIdToSeq = None
	proteinIdToTrueId = None
	proteinIds, proteinIdsMultiProc = prepareIdListFromInput(inputData)
	try:
		...
		try:
			...
			if FETCH_FROM_MIST and proteinIdToSeq:
				if SAVE_TO_FILE:
					...
					protNameToSeq = getChangedNamesAndSeqs(handle, proteinIdToSeq, proteinIdToTrueId)
